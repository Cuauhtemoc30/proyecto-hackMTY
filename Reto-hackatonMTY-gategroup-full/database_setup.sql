...
-- Setup completo con esquema y datos de ejemplo
-- ============================================================
...
/*
  ...
  1. Tablas
    - `products`: Productos de aerolínea
    - `drawers`: Cajones de vuelo con configuraciones
    - `employee_efficiency`: Registros de eficiencia de empleados
...
-- Políticas para drawers
DROP POLICY IF EXISTS "Allow public read access to drawers" ON drawers;
CREATE POLICY "Allow public read access to drawers"
  ON drawers FOR SELECT
  TO public
  USING (true);

DROP POLICY IF EXISTS "Allow public insert to drawers" ON drawers;
CREATE POLICY "Allow public insert to drawers"
  ON drawers FOR INSERT
  TO public
  WITH CHECK (true);

-- Políticas para employee_efficiency
DROP POLICY IF EXISTS "Allow public read access to employee_efficiency" ON employee_efficiency;
CREATE POLICY "Allow public read access to employee_efficiency"
  ON employee_efficiency FOR SELECT
  TO public
  USING (true);

DROP POLICY IF EXISTS "Allow public insert to employee_efficiency" ON employee_efficiency;
...
-- Nota: Para verificar los datos insertados, ejecuta estas consultas por separado:
-- SELECT COUNT(*) FROM products;
-- SELECT COUNT(*) FROM drawers;
-- SELECT COUNT(*) FROM employee_efficiency;
