...
import { Drawer } from '../lib/supabase';
import { fetchDrawers } from '../lib/mockData';
...
export default function DrawerAnalysis() {
  ...
  useEffect(() => {
    console.log('DrawerAnalysis component mounted, fetching drawers...');
    loadDrawers();
  }, []);
  ...
  const loadDrawers = async () => {
    setLoading(true);
    try {
      const data = await fetchDrawers();
      setDrawers(data);
    } catch (err) {
      console.error('Error in loadDrawers:', err);
      alert('Error al cargar cajones');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        {!loading && filteredDrawers.length === 0 && drawers.length === 0 && (
          <div className="text-center py-12">
            <Package className="w-12 h-12 text-slate-300 mx-auto mb-3" />
            <p className="text-slate-500 mb-2">No hay cajones en la base de datos</p>
            <p className="text-sm text-slate-400">Haz clic en "Insertar Datos" en la barra superior para agregar datos de ejemplo</p>
          </div>
        )}
